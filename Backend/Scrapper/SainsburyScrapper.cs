...
public async Task<List<string>> GetProductLinks(string brand, string product)
{
    ...
    try
    {
        ...
        // Wait for the product description elements to appear
        var selector = "h2.pt__info__description";
        ...
        {
            var linkElement = await element.QuerySelectorAsync("a.pt__link");
            if (linkElement != null)
            {
    ...
    public async Task<List<Product>> GetProductDetails(List<string> urls, string brand, string product)
{
    ...
    foreach (var url in urls)
    {
        try
        {
            ...
            productLocal.ProductName = await TryGetElementContent(page, "h1.pd__header[data-testid='pd-product-title']");
            ...
            {
                productLocal.Price = await TryGetElementContent(page, "[data-testid='pd-retail-price']");
                productLocal.pricePerUnit = await TryGetElementContent(page, "[data-testid='pd-unit-price']");
                productLocal.ImageUrl = await TryGetElementAttribute(page, "img.pd__image[data-testid='pd-selected-image']", "src");
    ...
    private async Task<string> TryGetElementContent(IPage page, string selector, int maxRetries = 3)
    {
    ...
    private async Task<string> TryGetElementAttribute(IPage page, string selector, string attribute, int maxRetries = 3)
    {
        for (int i = 0; i < maxRetries; i++)
        {
            try
